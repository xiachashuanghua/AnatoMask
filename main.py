...
parser.add_argument("--gpu_id", default=0, type=int, help="single GPU id for training")

parser.add_argument("--feature_size", default=48, type=int, help="feature size")
parser.add_argument("--pretrained_root", default="", help="pre-trained root")

parser.add_argument("--logdir", default="logs/runs_comer5_group", type=str, help="directory to save the tensorboard logs")
parser.add_argument("--data_dir", default="", type=str, help="dataset directory")
parser.add_argument("--cache_dir", default="", type=str, help="dataset directory")
parser.add_argument("--json_list", default="dataset_0.json", type=str, help="dataset json file")
parser.add_argument("--use_persistent_dataset", default=True, type=str2bool, help="use monai Dataset class")
...
parser.add_argument("--dist-backend", default=get_default_distributed_backend(), type=str, help="distributed backend")
...
parser.add_argument("--model_name", default="MiM",type=str, help="model name")
...
def main():
    ...
    if args.distributed:
        if os.name == "nt":
            raise RuntimeError("Distributed training is not supported on Windows in this first release.")
        args.ngpus_per_node = torch.cuda.device_count()
        print("Found total gpus", args.ngpus_per_node)
        args.world_size = args.ngpus_per_node * args.world_size
        mp.spawn(main_worker, nprocs=args.ngpus_per_node, args=(args,))
    else:
        main_worker(gpu=args.gpu_id, args=args)
...
def main_worker(gpu, args):
    ...
    if args.pretrained_root:
        ...
        checkpoint = torch.load(args.pretrained_root, map_location='cpu', weights_only=False)
        ckpt = {}
        for key, value in checkpoint['state_dict'].items():
            new_key = key.replace('module', 'vit')
            ckpt[new_key] = value
        out = model.load_state_dict(ckpt, strict=False)
        loaded_keys = [k for k in ckpt.keys() if k not in out.unexpected_keys]
        print(f"Successfully loaded {len(loaded_keys)} / {len(ckpt)} pretrained keys")
