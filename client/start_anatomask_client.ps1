...
$DefaultHost = if ($env:ANATOMASK_CLIENT_HOST) { $env:ANATOMASK_CLIENT_HOST } elseif ($env:ANATOMASK_HOST) { $env:ANATOMASK_HOST } else { "127.0.0.1" }
$DefaultPort = if ($env:ANATOMASK_CLIENT_PORT) { [int]$env:ANATOMASK_CLIENT_PORT } elseif ($env:ANATOMASK_PORT) { [int]$env:ANATOMASK_PORT } else { 7860 }
...
$RequirementsFile = Join-Path $ClientRoot "requirements.txt"
...
'@
    ...
    $tempScript = [System.IO.Path]::Combine($env:TEMP, "anatomask_client_runtime_check_$PID.py")
...
function Install-BasePackages {
    Write-Log "Installing desktop client dependencies from client\requirements.txt"
    & $script:UvBin pip install --python $EnvPython --link-mode copy -r $RequirementsFile
}
...
function Ensure-Runtime {
    ...
    if (Test-RuntimeReady) {
        Write-Log "AnatoMask desktop environment is ready. Launching the client."
        return
    }
...
function Launch-Client {
    ...
    Write-Log "Launching desktop client: $EnvPython -m client $($argsToUse -join ' ')"
    & $EnvPython -m client @argsToUse
}
