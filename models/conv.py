...
from models.vit_2d import PatchEmbed3d, CBlock3d, Mlp, MulAttention, RelativePositionBias3d
# , Block, CBlock3d
    ...
    def __init__(self, img_size=[512, 128, 64], patch_size=[4,2,2], in_chans=3, num_classes=80, embed_dim=[256,384,768], depth=[2,2,11],
        ...
        self.blocks4 = nn.ModuleList([
            Block(
                ...
                init_values=init_values, window_size=self.patch_embed4.patch_shape if use_rel_pos_bias else None, window_size_ds=[ps//2 for ps in self.patch_embed4.patch_shape] if sr != 1 else None, args=args)
...
class ConvViT3dClass(torch.nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=[512, 128, 64], patch_size=[4,2,2], in_chans=3, num_classes=80, embed_dim=[256,384,768], depth=[2,2,11],
                 num_heads=12, mlp_ratio=[4,4,4], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
    ...
    def forward(self, x, normalize=False):
        ...
        x = self.forward_features(x, normalize)
        x5 = x[-1]                                # [B, C, D', H', W']
        g = self.avgpool(x5).flatten(1)               # [B, C]
        logits = self.head(g)                         # [B, num_classes]
        return logits
def convvit3dclass(**kwargs):
    ''' 3d convvit with input size
    :96->48->24->12
    '''
    model = ConvViT3dClass(
        # in_chans=1,
        img_size=[[64, 64, 64], [32, 32, 32], [16, 16, 16], [8, 8, 8]],
        patch_size=[2, 2, 2, 2],
        embed_dim=[48, 96, 192, 384, 768],
        depth=[2, 2, 2, 11],
        num_heads=12,
